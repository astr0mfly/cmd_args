...
struct option_value : option
{
    ...
    option_value(char sname, std::string lname, std::string help, std::string type, std::string value)
...
struct option_action : option
{
    ...
    option_action(char sname, std::string lname, std::string help, std::function<void(void)> callback)
        : option(sname, std::move(lname), (std::move(help)))
...
public:
    ...
    {
        auto it =
            std::find_if(m_vecValues.cbegin(), m_vecValues.cend(), [_Key](decltype(m_vecValues)::const_reference _V) {
                return _V.short_name == _Key;
            });
    ...
    {
        m_vecValues.emplace_back(sname, std::move(lname), std::move(help), std::move(type), std::move(value));
    }

    void push(std::string lname, std::string help, std::string type, std::string value)
    {
        m_vecValues.emplace_back(std::move(lname), std::move(help), std::move(type), std::move(value));
    }
