...
struct position_argument : named_argument {
    ...
    position_argument(size_t _Seq, std::string _Name, std::string _Type, std::string _Help)
        ...
        , iSeq(_Seq) {}
    bool operator<(position_argument const& _Rhs) const noexcept {
        return iSeq < _Rhs.iSeq;
    }
};
...
public:
    bool set(std::string const& line) {
        ...
        if (!it->second.hasValue()) {
            std::cerr << "(parse error) named_argument " << it->second.strName
    ...
    bool doEmpty() const noexcept override {
        return mapArgs.empty();
    }
    size_t doSize() const noexcept override {
        return mapArgs.size();
    }
    void doVisit(std::function<void(argument const&)> _Fn) const override try {
    ...
    void doGetHelp(std::ostringstream& _Out, std::size_t _Max) const override {
        ...
        for (auto const& arg : mapArgs) {
            ...
            _Out << std::string(_Max - arg.second.strName.length(), ' ') << "("
                 << arg.second.strType << ") ";
...
public:
    ...
    bool doFind(std::string const& _Key) const noexcept override {
        return std::find_if(mapArgs.cbegin(), mapArgs.cend(),
                            [&_Key](decltype(mapArgs)::const_reference arg) {
                                return arg.second.strName == _Key;
                            }) != mapArgs.cend();
    }
    ...
    argument const* doGet(std::string const& _Key) const override {
        auto it = std::find_if(
            mapArgs.begin(), mapArgs.end(),
            [&](decltype(mapArgs)::const_reference _V) { return _V.second.strName == _Key; });
        if (it != mapArgs.end()) return nullptr;
